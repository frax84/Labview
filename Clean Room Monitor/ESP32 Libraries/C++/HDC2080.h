...
class HDC2080 {
  public:
    HDC2080();
    HDC2080(const int address);
    bool isDataReady();
	bool softReset();
	bool configMode(hdc2080_autoreadmode_t readMode);
	bool configMeas(hdc2080_bitres_t temp_res, hdc2080_bitres_t hum_res, hdc2080_read_conf_t read_conf);
	bool read(uint16_t &temp, uint16_t &hum);
  private:
    int _address;
};
